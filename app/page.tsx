...
import { mockIntelligenceInsights } from '@/lib/mock-data';
import { uploadDocument, startAnalysis, getJobStatus, getAnalysisResult } from '@/lib/api';
import type { DocumentAnalysis, IntelligenceInsight } from '@/lib/types';
...
export default function Home() {
  ...
  const [insights, setInsights]             = useState<IntelligenceInsight[]>(mockIntelligenceInsights);
      ...
      const tick = async () => {
        try {
          ...
          if (status.status === 'complete') {
            // Fetch full result
            const { analysis, insights: newInsights } = await getAnalysisResult(documentId);
  ...
  return (
    <DashboardLayout insights={insights}>
